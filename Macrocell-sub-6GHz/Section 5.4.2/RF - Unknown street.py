...
X_test = df_test.iloc[:, [12,5,7,10,4,15,9,8,18,20,19,16,17,13]] #Predictors
...
rf_regressor =  RandomForestRegressor(n_estimators=44, max_depth=3, min_samples_leaf=3,criterion='squared_error', random_state=42)
...
plt.scatter(y_target_desn_test,y_pred_desn_test,label='Result for Route #2 ($R^2:0.31)')
plt.grid(ls='--')
plt.legend()
...
max_depth = np.arange(1, 4, 1) #define maximum depth
...
matrix_cv_rf = np.zeros((p, 5)) #size of the cross-validation matriz
...
for i in range(max_iter):
    ...
    for k in range(len(num_trees)): 
        ...
        matrix_cv_rf[count][3] = RMSE_tot_train
        matrix_cv_rf[count][4] = RMSE_tot_val
        count += 1
        c +=1
