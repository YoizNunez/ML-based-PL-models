...
n=1000 #the first 1000 samples of the route Jardim Botanico
df_train= df.iloc[:n]

n=374#the last 374 samples of the route Jardim Botanico
df_test= df.tail(n)
...
plt.scatter(d,pl)
plt.grid(ls='--')
plt.xlabel('d [m]',size=12)
plt.ylabel('Measured PL [dB]',size=12)
...
rf_regressor =  RandomForestRegressor(n_estimators=14, max_depth=3, min_samples_leaf=10,criterion='squared_error', random_state=42)
