...
"""
X = df.iloc[:, [2,3,4]] #with vegetation depth -->5 
y = df.iloc[:, [5]]
...
gtb_regressor = GradientBoostingRegressor(n_estimators=50, 
                                          learning_rate=0.1,
                                          max_depth=5, 
                                          min_samples_leaf=15, 
                                          alpha=0.9, 
...
max_depth = list(range(2,8)) #define maximum depth
    
min_samples_leaf = list(range(1,20)) #define minumum samples leaf

max_iter = len(max_depth)*len(min_samples_leaf)  #number of iteractions

iteration = list(range(1,max_iter+1))

num_trees = np.arange(8, 100, 4) #number of trees
