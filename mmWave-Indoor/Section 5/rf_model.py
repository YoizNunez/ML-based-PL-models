...
for Table 6 for each predictors coalitions for the RF model
"""
...
def rf_model_IML(df):
    ...
    print("----Calculating performance indicators RF model---")
    
    """
    training the ML model
    """
    def rf(X_train,y_train):
      rf_regressor = RandomForestRegressor(n_estimators=178, max_depth=6 , max_features = 'auto', min_samples_split=2, min_samples_leaf=1, random_state=0)
    ...
    """
    def path_loss_rf(X_train,y_train,X_test,y_test):
    
      rf_regressor = rf(X_train,y_train)
      rf_regressor.fit(X_train,np.ravel(y_train))
    ...
    for i in range(len(p)):  
      ...
      y_target_train,y_target_test,y_pred_train,y_pred_test = path_loss_rf(X_train,y_train,X_test,y_test)
    ...
    rf_results={"Pred1":[pred1,rmse_p1,mape_p1,SD_p1,R2_p1],
    ...
    """
    print("----Calculating Mutual Information RF model---")
    X = df.iloc[:, [0,1,2,3,4]]
    y = df.iloc[:, -1]
    ...
    #Calculating JMI
    jmi_2= knn.cmi(['d','nw'],['PL'],[],30, df1)
    jmi_3= knn.cmi(['d','nw','f'],['PL'],[],33, df1) 
    jmi_4= knn.cmi(['d','nw','f','dy'],['PL'],[],34, df1) 
    jmi_5= knn.cmi(['d','nw','f','dy','dx'],['PL'],[],31, df1) 
     
    #Calculating CMI
    #cmi(X;Y|Z), mutual information between X and Y, conditioned in Z. 
    cmi_2= knn.cmi(['PL'],['nw'],['d'],46, df1) 
    cmi_3= knn.cmi(['PL'],['f'],['d','nw'],50, df1) 
    cmi_4= knn.cmi(['PL'],['dy'],['d','nw','f'],55, df1)
    cmi_5= knn.cmi(['PL'],['dx'],['d','nw','f','dy'],43, df1)
     
    mi_results_rf={"Subset 2 predictors":[jmi_2,cmi_2],
    ...
    """
    print("----Calculating IML indicators RF model---")

    y = df.iloc[:, -1]
    
    #preparing the input's model for each subset
    pred1 = rf_results["Pred1"][0]
    pred2 = rf_results["Pred2"][0]
    pred3 = rf_results["Pred3"][0]
    pred4 = rf_results["Pred4"][0]
    pred5 = rf_results["Pred5"][0]
    ...
    #name of predictors
    features_list_p1=['d']
    features_list_p2=['d','nw']
    features_list_p3=['d','nw','f']
    features_list_p4=['d','nw','f','dy']
    features_list_p5=['d','nw','f','dy','dx']
    ...
    def IML(X_train,y_train,X_train_df,features_list):
        ...
        pickle.dump(rf_regressor, open('rfmodel_IML.pkl','wb')) #save the model, i.e., 'mlpmodel_IML_5.pkl' for the subset of 5 predictors
        rf = pickle.load(open('rfmodel_IML.pkl','rb')) #the file is load from the local path. In my case from C:/Users/Yoiz Nuñez
        rf_model = ('RF',rf)
        
        #ALE
        explainer_rf = pymint.InterpretToolkit(rf_model,X=X_train_df, y=y_train)
        ale_model = explainer_rf.ale(features=features_list, n_bins=30, n_jobs=1, subsample=1.0, n_bootstrap=1)
            
        MEC = explainer_rf.main_effect_complexity(ale_model,max_segments=10)
        
        IAS= explainer_rf.interaction_strength(ale_model)
    ...
    #predictor d
    ale_d_value_s1 = ale_model.d__RF__ale.to_masked_array()  
    ...
    MEC_p2= MEC_p2['RF']
    IAS_p2=IAS_p2.RF_ias[0] #to get the float value from the dataarray.DataArray result
    IAS_p2 = IAS_p2.to_masked_array()
    IAS_p2=IAS_p2.tolist() 
    
    #predictor d
    ale_d_value_s2 = ale_model.d__RF__ale.to_masked_array()  
    ale_d_value_s2= ale_d_value_s2.ravel() #to reduce 1 dim
    ale_d_bin_s2 = ale_model.d__bin_values.to_masked_array()
    
    #predictor nw
    ale_nw_value_s2 = ale_model.nw__RF__ale.to_masked_array()  
    ...
    ALE_results = {"Subset 1":[ale_d_value_s1,ale_d_bin_s1],
                   "Subset 2":[ale_d_value_s2,ale_d_bin_s2,ale_nw_value_s2,ale_nw_bin_s2],
                   "Subset 3":[ale_d_value_s3,ale_d_bin_s3,ale_nw_value_s3,ale_nw_bin_s3,ale_f_value_s3,ale_f_bin_s3],
                   "Subset 4":[ale_d_value_s4,ale_d_bin_s4,ale_nw_value_s4,ale_nw_bin_s4,ale_f_value_s4,ale_f_bin_s4,ale_dy_value_s4,ale_dy_bin_s4],
                   "Subset 5":[ale_d_value_s5,ale_d_bin_s5,ale_nw_value_s5,ale_nw_bin_s5,ale_f_value_s5,ale_f_bin_s5,ale_dy_value_s5,ale_dy_bin_s5,ale_dx_value_s5,ale_dx_bin_s5],
                     }
      
    IML_results_rf={"Subset 1 predictor":[IAS_p1,MEC_p1],
    ...
    """
    print("----Calculating PFI RF model---")
    ...
    rf_regressor = rf(X_train_p2,y_train_p2)
    baseline, scores= PermImportance(X_train_df_p2,y_train_p2,rf_regressor,num_iterations=10)
    ...
    pred2.append(scores['nw'])
    PFI_pred2= pred2
    PFI_pred2_s2=np.mean(PFI_pred2)
    
    #subset of 3 predictors
    rf_regressor = rf(X_train_p3,y_train_p3)
    baseline, scores= PermImportance(X_train_df_p3,y_train_p3,rf_regressor,num_iterations=10)
    ...
    rf_regressor = rf(X_train_p4,y_train_p4)
    baseline, scores= PermImportance(X_train_df_p4,y_train_p4,rf_regressor,num_iterations=10)
    ...
    rf_regressor = rf(X_train_p5,y_train_p5)
    baseline, scores= PermImportance(X_train_df_p5,y_train_p5,rf_regressor,num_iterations=10)
    ...
    PFI_results_rf={"Subset 2 predictors":[PFI_pred1_s2,PFI_pred2_s2],
    ...
    """
    print("----Final model with optimized predictors subsets RF model---")
    
    y_target_train,y_target_test,y_pred_train,y_pred_test = path_loss_rf(X_train_p4,y_train_p4,X_test_p4,y_test_p4)
    RMSE_train, MAPE_train, SD_train, R2_train = functions_tools.statistics(y_target_train, y_pred_train)
    ...
    return rf_results,mi_results_rf,IML_results_rf,PFI_results_rf,ALE_results,final_model